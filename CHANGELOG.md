...
- **`mcp-server/src/edit/`** — six new modules (`revision.ts`, `normalize.ts`, `block-parser.ts`, `fuzzy.ts`, `errors.ts`, `ops.ts`) plus `mcp-server/src/lib/vault-readonly.ts` shared helper. 72 unit tests cover surrogate pairs, CJK-punct invariant, code-block exclusion, multi-op atomicity, EOF newline rule, empty-content bootstrap, and `REVISION_MISMATCH` recovery.
