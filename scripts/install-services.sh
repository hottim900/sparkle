...
cp "$SERVICE_DIR/capture-hub-portproxy.service" /etc/systemd/system/
...
systemctl enable capture-hub-portproxy.service

# 立即啟動
systemctl start capture-hub.service
systemctl start capture-hub-tunnel.service
systemctl start capture-hub-portproxy.service
...
echo ""
systemctl status capture-hub-portproxy.service --no-pager -l | head -5
echo ""
echo "常用指令："
echo "  查看狀態:  sudo systemctl status capture-hub"
echo "  查看 log:  journalctl -u capture-hub -f"
echo "  重啟:      sudo systemctl restart capture-hub"
echo "  全部重啟:  sudo systemctl restart capture-hub capture-hub-tunnel capture-hub-portproxy"
