...
export async function fetchWithRetry(url: string, options: RequestInit = {}): Promise<Response> {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      // CF Access session expired: network error while browser is online.
      // Skip retries — CF Access blocks at the edge, retrying won't help.
...
export async function releaseVaultStub(
  ...
  return request<{ ok: boolean; id: string; vault_path: string | null }>(
    `/items/${id}/vault-stub`,
    { method: "DELETE" },
  );
}
