...
src/
  App.tsx               # Router setup (RouterProvider + AuthGate)
  routeTree.gen.ts      # Auto-generated by TanStack Router Vite plugin
  routes/
    __root.tsx          # Root layout (sidebar, bottom-nav, search overlay, keyboard shortcuts, AppContext)
    index.tsx           # Redirect → /notes/fleeting
    _list.tsx           # Pathless layout for list routes (split panel + detail via ?item=)
    _list/
      notes.tsx         # Notes layout (sub-nav chips: 閃念/發展中/永久筆記/已匯出)
      notes/{fleeting,developing,permanent,exported,index}.tsx  # Note list pages
      todos.tsx         # Todos layout (sub-nav chips: 進行中/已完成)
      todos/{index,done}.tsx  # Todo list pages
      scratch/index.tsx # Scratch list page
      all.tsx           # All items page
      archived.tsx      # Archived items page
    dashboard.tsx       # Full-width dashboard route
    settings.tsx        # Full-width settings route
    shares.tsx          # Full-width share management route
  components/
    ...
    loading-fallback.tsx # Shared loading spinner (route Suspense fallback)
  ...
  lib/
    ...
    app-context.ts      # AppContext + useAppContext hook (obsidianEnabled, isOnline)
    types.ts            # TypeScript interfaces + ShareToken types
    navigation.ts       # ViewType↔path mapping (pathToView, viewToPath, isListRoute)
    search-params.ts    # Zod schemas for route search params (rootSearchSchema, listSearchSchema)
    __tests__/           # Lib unit tests (fetchWithRetry, navigation, search-params)
  hooks/
