...
## Claude.ai MCP HTTP 伺服器（選用）

Sparkle 可透過遠端 HTTP transport 搭配 OAuth 2.0 認證，讓 Claude.ai 網頁版存取 MCP 工具。

### 前置條件

- Cloudflare Tunnel 搭配**獨立 hostname** 給 MCP（例如 `sparkle-mcp.your-domain.com`）
- MCP hostname **不可**設定 Cloudflare Access（Claude.ai 伺服器需要直接存取）
- 使用單層子網域（例如 `sparkle-mcp.domain.com`，而非 `mcp.sparkle.domain.com`）— 免費 SSL 只覆蓋 `*.domain.com`
...
```bash
SPARKLE_AUTH_TOKEN=同上方的AUTH_TOKEN
MCP_HTTP_PORT=3001
MCP_AUTH_PIN=你的密碼
...
4. **新增 Cloudflare Tunnel 路由**（tunnel 設定檔）：
...
5. **新增 DNS 紀錄**：CNAME `sparkle-mcp` → 你的 tunnel ID

6. **在 Claude.ai 連接**：Settings → Connectors → Add custom connector → `https://sparkle-mcp.your-domain.com/mcp`

系統提示輸入 PIN 時，輸入你設定的 MCP_AUTH_PIN。

### 運作方式

- OAuth 2.0 Authorization Code flow（含 PKCE）
- PIN 閘控授權（防暴力破解，最多 5 次嘗試）
- JWT access token，簽名金鑰由 MCP_AUTH_PIN 衍生
- Token 可跨重啟存活（stateless 驗證，1 年有效期）
- `sparkle-mcp-http.service` 依賴 `sparkle.service`（MCP 透過 localhost 呼叫 Sparkle API）
